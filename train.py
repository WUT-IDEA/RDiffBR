...
############################################################################################################## init_diff
        ...
        Diff = diff_model.Diff(world.config, dataset)
        Diff = Diff.to(world.device)
        ...
        bundle_reverse_model = register.DIFF_MODELS['dnn'](in_dims, out_dims, args.emb_size, time_type="cat",
                                                           norm=args.norm)
        bundle_reverse_model = bundle_reverse_model.to(world.device)

        opt_bundle_dnn = optim.Adam(bundle_reverse_model.parameters(), lr=0.001, weight_decay=1.0e-4)
        ...
        diffusion = gd.GaussianDiffusion(world.config, mean_type, args.noise_schedule, \
         args.noise_scale, args.noise_min, args.noise_max, args.steps, world.device).to(
            world.device)


        ##############################################################################################################
        #model.load_state_dict(torch.load('./ED_1_Neg_1_2048_0.001_0.0001_64_0.2_0.2_0.2_1_0.04_0.25'), strict=False)
        #bundle_reverse_model.load_state_dict(torch.load('./df'))
       ...
       #     param.requires_grad = False  #不更新
        ...
        for epoch in range(conf['epochs']):
            ...
            for batch_i, batch in pbar:
                ...
                bpr_loss, c_loss,IL_bundles_emb = model(batch, ED_drop=ED_drop)


                df_loss = Diff(batch, bundle_reverse_model, diffusion,IL_bundles_emb)


                optimizer.zero_grad()
                opt_bundle_dnn.zero_grad()
                ...
                diff_loss = df_loss.detach()

                run.add_scalar("loss_bpr", bpr_loss_scalar, batch_anchor)
                run.add_scalar("loss_c", c_loss_scalar, batch_anchor)
                run.add_scalar("loss_diff", diff_loss, batch_anchor)
                run.add_scalar("loss", loss_scalar, batch_anchor)

                pbar.set_description("epoch: %d, loss: %.4f, bpr_loss: %.4f, c_loss: %.4f,diff_loss: %.8f" %(epoch, loss_scalar, bpr_loss_scalar, c_loss_scalar,diff_loss))

                if (batch_anchor+1) % test_interval_bs == 0:
                    metrics = {}
                    metrics["val"] = test(model,Diff,bundle_reverse_model, diffusion,dataset.val_loader, conf,bi_conf='val')
                    metrics["test"] = test(model,Diff,bundle_reverse_model, diffusion,dataset.test_loader, conf,bi_conf='test')
                    best_metrics, best_perform, best_epoch = log_metrics(conf, model, bundle_reverse_model,metrics, run, log_path, checkpoint_model_path, checkpoint_conf_path, epoch, batch_anchor, best_metrics, best_perform, best_epoch)
...
def  log_metrics(conf, model,bundle_reverse_model, metrics, run, log_path, checkpoint_model_path, checkpoint_conf_path, epoch, batch_anchor, best_metrics, best_perform, best_epoch):
    for topk in conf["topk"]:
    ...
    if metrics["val"]["recall"][topk_] > best_metrics["val"]["recall"][topk_] and metrics["val"]["ndcg"][topk_] > best_metrics["val"]["ndcg"][topk_]:
        ...
        torch.save(bundle_reverse_model.state_dict(),'./df')
        #torch.save(model, './crosscbr0.5.pth')
        dump_conf = dict(conf)
...
def test(model,diff,bundle_reverse_model,diffusion, dataloader, conf,bi_conf):
    tmp_metrics = {}
    ...
    device = conf["device"]
    model.eval()
    bundle_reverse_model.eval()
    rs = model.propagate(bi_conf,test=True)

    Denoise_IL_bundle_feature = diff.get_scores(bundle_reverse_model, diffusion, rs[1][0])

    for users, ground_truth_u_b, train_mask_u_b in dataloader:

        pred_b = model.evaluate(rs,Denoise_IL_bundle_feature,users.to(device))
...
def get_metrics(metrics, grd, pred, topks):
    ...
    for topk in topks:
        ...
        col_indice=col_indice.to(device)

        row_indice = torch.zeros_like(col_indice) + torch.arange(pred.shape[0], device=pred.device, dtype=torch.long).view(-1, 1).to(device)

        is_hit = grd[row_indice.view(-1), col_indice.view(-1)].view(-1, topk)

        tmp["recall"][topk] = get_recall(pred, grd, is_hit, topk)
        tmp["ndcg"][topk] = get_ndcg(pred, grd, is_hit, topk)
        #print(tmp["recall"][topk], tmp["ndcg"][topk])
