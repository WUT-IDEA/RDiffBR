...
class BundleTestDataset(Dataset):
    def __init__(self, u_b_pairs, u_b_graph, u_b_graph_train, num_users, num_bundles):
        self.u_b_pairs = u_b_pairs     #没用
        self.u_b_graph = u_b_graph
        self.train_mask_u_b = u_b_graph_train

        self.num_users = num_users     #没用
        self.num_bundles = num_bundles   #没用

        self.users = torch.arange(num_users, dtype=torch.long).unsqueeze(dim=1)  #没用
        self.bundles = torch.arange(num_bundles, dtype=torch.long)   #没用
...
class Datasets():
    def __init__(self, conf):
        ...
        b_i_graph_mask = self.get_bi_mask()
       ...
       # self.graphs = [u_b_graph_train, u_i_graph, b_i_graph]
        self.graphs = [u_b_graph_train, u_i_graph, b_i_graph, b_i_graph_mask]
    ...
    def get_bi(self):  #for train
        with open(os.path.join(self.path, self.name, 'Fold_bundle_item_mask0.5.txt'), 'r') as f:
    ...
    def get_bi_mask(self):    ##############################   0 10，20，30，40，60，70，80，90% mask
        ...
        print_statistics(b_i_graph, 'B-I test statistics')
